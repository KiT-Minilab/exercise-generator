...
func loadDefaultConfig() *Config {
	return &Config{
		...
		OpenaiClient: OpenaiClient{
			...
			UserMessageTemplate: "Give a single choice question with {{.QuestionType}} type to help student learn word **{{.Word}}**. Provide the question in JSON format with keys: `questionStem`, `questionType`, `answers`, `distractors`. Note that, there must be only one correct answer (length of `answers` array is 1), `distractors` must be 3 wrong choices, the word **{{.Word}}** must appear in field `questionStem` and not appear in 2 fields `answers` and `distractors`.",
			MaxTokens:           256,
			Temperature:         1,
			TopP:                1,
		},
